...
public class CreatePayment
{
    ...
    /// <summary>
    /// Устарело (deprecated). Режим определяется API-ключом: Test-ключ всегда создаёт тестовый платёж, Live-ключ — боевой.
    /// Значение true временно принимается только с Live-ключом для совместимости. Новым интеграциям поле передавать не нужно.
    /// </summary>
    [Obsolete("Режим определяется API-ключом (Test/Live). Поле устарело и будет удалено; не передавайте его в новых интеграциях.")]
